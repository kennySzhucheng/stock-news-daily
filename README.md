...
- 本工具不做的事：
  - 不给出"买入/卖出"**指令**。给出的是**带证伪条件的候选观察清单**：每条候选必须写明
    "什么情况算这条判断错了"，且**它后来的真实表现会被公开回填，含跑输的**
  - 不保证新闻解读正确，所有 AI 结论标注置信度
  - 不接入实盘交易

> 之所以仍不算"荐股"：候选清单里每条都带**可观测的推翻条件**，1/3/5 天后按相对沪深300
> 的超额逐条回填、不删跑输的。它是一份**可检验的判断记录**，而不是一份"买这几只"的指令。
> 呈现上只给平均超额与逐条明细，**不给"胜率"** —— 个位数样本下的百分比没有意义，
> 且会把连续的超额压成二值的对/错。
...
│   [M10 候选复盘] → DeepSeek：从当日判断里挑出候选观察清单，锁基准价；
│                    并对历史候选回填 1/3/5 日后相对沪深300 的真实超额
...
| 版块顺序 | 摘要 → **市场分析** → 候选清单 → 个股行情 → 分板块新闻 | 总览 / 新闻 / 行情 / 板块 / 深度分析 / 候选 / 历史 |
...
| M10 | 候选观察清单 + 事后复盘 | M2, M3, M4 | ✅ 2026-09-24 · 三处展示；收盘闸门 + 到期即补打分 |
...
│   └── m10_picks/     ← 候选观察清单与事后复盘（picks.py，含 --probe 自检）
├── data/              ← 运行时数据（不入库）
└── reports/           ← 生成的日报（不入库，部署到 gh-pages 分支）
    ...
    ├── web/                 ← M9 静态导出的网页版
    └── picks/ledger.jsonl   ← M10 候选账本（跨天累积，靠 gh-pages 留存，见 docs/tasks.md）
```
